...
def process_person_data(name, data):
    ...
    try:
        ...
        try:
            ...
            local_dt = datetime(*birth_time, tzinfo=local_tz)
        ...
        person_details = {
            ...
            "cross_name": hd_data.get("cross_name", ""),
            "quarter": hd_data.get("quarter", {}),
            "line_counts": hd_data.get("line_counts", {}),
            "sun_roles": hd_data.get("sun_roles", {}),
            "yin_yang_balance": hd_data.get("yin_yang_balance", {}),
            "contour": hd_data.get("contour", {}),
...
def process_hybrid_analysis(participants, group_type="family", verbosity="all"):
    ...
    # Get pysweph version if possible, or swisseph lib version
    # swe.swe_version() might require path/args. swe.version() isn't standard function name in pysweph 2.x?
    # Actually `swe` module from pysweph doesn't always expose version string easy.
    # But `swe.swe_calc_ut` relies on underlying dll.
    # We'll hardcode "pysweph" and dynamic timestamp for now or try-catch version.
    ephemeris_ver = "SwissEph (pysweph)" 
